...
|----------|-------------|----------|---------|---------|
| `KNXD_VERSION` | Version/tag of knxd to build | **Yes** | - | `0.14.75` |
...
```bash
docker build --build-arg KNXD_VERSION=0.14.75 -t knxd .
```
...
```bash
docker build \
    --build-arg KNXD_VERSION=0.14.75 \
    --build-arg BUILD_DATE=$(date -u +'%Y-%m-%dT%H:%M:%SZ') \
    --build-arg VCS_REF=$(git rev-parse --short HEAD) \
    -t michelmu/knxd-docker:latest \
    -t michelmu/knxd-docker:0.14.75 \
    .
```
